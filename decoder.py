...
class CaptioningModel(nn.Module):
    ...
    def forward_one(self, batch, return_info=False):
        ...
        # print("done with visual features: ",visual_features.shape)
    ...
    def get_video_logits(self, batch):
        # shape: (batch_size, max_caption_length, vocab_size)
        if 'image' in batch:
            if isinstance(batch['image'], (list, tuple)):
                features = [self.image_encoder(im) for im in batch['image']]
        ...
        search_param = {"reture_logit":True}
        # the start_predictions are not in predicted_caption
        # print(f"start_predictions: {start_predictions}")
        # print(f"start_predictions.shape: {start_predictions.shape}")
        predicted_caption, logprobs, all_scores_record = self.decoder.search(
            start_predictions, decoding_step, **search_param
        )
        if 'prefix' in batch:
            # we need to remove prefix from predicted_caption
            predicted_caption = predicted_caption[:, start_predictions.shape[1]:]
        for i in range(len(all_scores_record)):
            all_scores_record[i] = all_scores_record[i].to("cpu")
        output_dict = {
            'predictions': predicted_caption.to("cpu"),
            'logprobs': logprobs.to("cpu"),
            "logits":all_scores_record
        }
        return output_dict
    ...
    def search(
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                if reture_logit:
                    all_scores_record.append(scores)
        ...
        if reture_logit:
            return decoded, logprobs, all_scores_record
        else:
